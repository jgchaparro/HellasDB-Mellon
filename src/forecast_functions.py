...
def correct_forecasts(ts, 
    ...
    for p in ps:
        for d in ds:
            for q in qs:
                if (p, d, q) in exclude[region_name]:
                    pass
                else:
                    for tr in trends:
...
# =============================================================================
    ...
    # Generate splits    
    break_time = dt(break_year, 12, 31)
    start_time = dt(start_year, 12, 31)
    
    # Endogenous variable
    endog = endog[endog.index >= start_time]
    endog_train = endog[(endog.index >= start_time) &
                        (endog.index <= break_time)]
